...
type ChatConfig struct {
	...
	TopP             float32  // 0.0 t0 1.0 - to a low value, like 0.1 the model will be very conservative in the future
	N                int      // number of message to generate
	Stop             []string // list of tokens to stop on
	MaxTokens        int      // number of tokens to generate
	PresencePenalty  float32  // -2.0 to 2.0  - Number between -2.0 and 2.0. Positive values penalize new tokens
	FrequencyPenalty float32  // -2.0 to 2.0 - Number between -2.0 and 2.0. Positive values penalize new tokens
}
